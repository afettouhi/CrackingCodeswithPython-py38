...
def main():
    ...
    # from a file.
    filename = '../data/encrypted_file.txt'  # The file to write to/read from.
    mode = 'encrypt'  # Set to either 'encrypt' or 'decrypt'.

    if mode == 'encrypt':
        message = 'Journalists belong in the gutter because that is where the ruling classes throw their guilty ' \
                  'secrets. Gerald Priestland. The Founding Fathers gave the free press the protection it must have ' \
                  'to bare the secrets of government and inform the people. Hugo Black. '
        pubKeyFilename = '../data/al_sweigart_pubkey.txt'
        ...
        privKeyFilename = '../data/al_sweigart_privkey.txt'
...
def encryptAndWriteToFile(messageFilename, keyFilename, message, blockSize=None):
    ...
    if not (math.log(2 ** keySize, len(SYMBOLS)) >= blockSize):
        sys.exit(
            'ERROR: Block size is too large for the key and symbol set size. Did you specify the correct key file and '
            'encrypted file?')
    # Encrypt the message:
